...
auto CPU::Xasan::poison(u32 paddr, u32 size, u8 poisonType) -> void {
  ...
  for(u32 g = first; g <= last && g < shadow.size(); g++) {
    ...
    n6 permissions = shadow[g].permissions();
    if(segmentStart == granuleStart && segmentEnd == granuleEnd) {
      shadow[g] = ShadowEntry::create(permissions, poisonType, 0);
      continue;
    }
    if(segmentEnd == granuleEnd) {
      u8 tailInvalidBytes = granuleEnd - segmentStart;
      shadow[g] = ShadowEntry::create(permissions, poisonType, tailInvalidBytes);
      continue;
    }
    shadow[g] = ShadowEntry::create(permissions, poisonType, 0);
  }
...
auto CPU::Xasan::unpoison(u32 paddr, u32 size) -> void {
  ...
  for(u32 g = first; g <= last && g < shadow.size(); g++) {
    ...
    u64 segmentEnd = rangeEnd < granuleEnd ? rangeEnd : granuleEnd;
    if(segmentStart == granuleStart && segmentEnd == granuleEnd) {
      shadow[g] = ShadowEntry::create(entry.permissions(), PoisonAccessible, 0);
      continue;
    }
    if(segmentStart == granuleStart) {
      ...
      shadow[g] = ShadowEntry::create(entry.permissions(), poisonType, tailInvalidBytes);
      continue;
    }
    shadow[g] = ShadowEntry::create(entry.permissions(), PoisonAccessible, 0);
  }
...
auto CPU::Xasan::setStackPermission(u32 paddr, u32 size, bool allowed) -> void {
  if(!size) return;
  if(shadow.empty()) shadow.resize(RdramSize / Granule, defaultEntry());
  u64 rangeStart = paddr;
  u64 rangeEnd = rangeStart + size;
  if(rangeEnd > RdramSize) rangeEnd = RdramSize;
  if(rangeEnd <= rangeStart) return;
  ...
  for(u32 g = first; g <= last && g < shadow.size(); g++) {
    ShadowEntry entry = shadow[g];
    u8 permissions = entry.permissions();
    if(allowed) {
...
auto CPU::Xasan::enableCheckBits(u8 bits) -> u8 {
  checkMask |= bits & CheckMaskAll;
  return checkMask;
}

auto CPU::Xasan::disableCheckBits(u8 bits) -> u8 {
  checkMask &= ~(bits & CheckMaskAll);
  return checkMask;
}
...
auto CPU::Xasan::checkRead(CPU& self, const PhysAccess& access, u32 size, u8 asanAccessType) -> bool {
  if(access.paddr >= RdramSize) return true;
  u8 accessType = asanAccessType ? asanAccessType : AccessTypeCpuRead;
  if(accessType > AccessTypeRspDmaWrite) accessType = AccessTypeCpuRead;
  if(!(checkMask & (1 << accessType))) return true;
  u8 permissionMask = PermissionCpuRead;
  if(accessType == AccessTypeStack) permissionMask |= PermissionStack;
  auto fault = check(access.paddr, size, permissionMask, accessType);
  ...
  if(self.emuxState.excMask.bit(ExceptionMaskBit)) {
    if(fault.accessType == AccessTypeStack) {
      disableCheckBits((1 << AccessTypeStack) | (1 << AccessTypeCpuRead) | (1 << AccessTypeCpuWrite));
    }
...
auto CPU::Xasan::checkWrite(CPU& self, const PhysAccess& access, u32 size, u8 asanAccessType) -> bool {
  if(access.paddr >= RdramSize) return true;
  u8 accessType = asanAccessType ? asanAccessType : AccessTypeCpuWrite;
  if(accessType > AccessTypeRspDmaWrite) accessType = AccessTypeCpuWrite;
  if(!(checkMask & (1 << accessType))) return true;
  u8 permissionMask = PermissionCpuWrite;
  if(accessType == AccessTypeStack) permissionMask |= PermissionStack;
  auto fault = check(access.paddr, size, permissionMask, accessType);
  if(!fault.active()) return true;
